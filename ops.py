...
def spectral_norm(w: Tensor, iteration=1):
...
def instance_norm(x, scope='instance_norm'):
    return tf_contrib.layers.instance_norm(x, epsilon=1e-05,
...
def fully_connected_with_w(x, use_bias=True, sn=False, reuse=False, scope='linear'):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if sn:
            ...
            x = tf.layers.dense(x, units=1, kernel_initializer=weight_init, kernel_regularizer=weight_regularizer,
                                use_bias=use_bias)
...
def adaptive_instance_layer_norm(x: Tensor, gamma, beta, smoothing=True, scope='instance_layer_norm'):
    with tf.variable_scope(scope):
        ch = x.shape[-1]
        eps = 1e-5
        ...
        if smoothing:
            rho = tf.get_variable('rho', [ch], initializer=tf.constant_initializer(0.9),
                                  constraint=lambda a: tf.clip_by_value(a, clip_value_min=0.0, clip_value_max=0.9))
        else:
            rho = tf.get_variable('rho', [ch], initializer=tf.constant_initializer(0.9),
                                  constraint=lambda a: tf.clip_by_value(a, clip_value_min=0.0, clip_value_max=0.9))
...
def layer_instance_norm(x: Tensor, scope='layer_instance_norm'):
    with tf.variable_scope(scope):
        ch = x.shape[-1]
        eps = 1e-5
        ...
        ln_mean, ln_sigma = tf.nn.moments(x, axes=[1, 2], keep_dims=True)
        x_ln = (x - ln_mean) / tf.sqrt(ln_sigma + eps)

        rho = tf.get_variable('rho', [ch], initializer=tf.constant_initializer(0.0),
                              constraint=lambda a: tf.clip_by_value(a, clip_value_min=0.0, clip_value_max=1.0))
...
def up_sample(x: Tensor, scale_factor=2):
...
def cam_loss(source, non_source):
    identity_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(tf.ones_like(source)), logits=source)
    non_identity_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(tf.zeros_like(non_source),
                                                                               logits=non_source))
...
def generator_loss(loss_func, fake):
    ...
    for i in range(2):
        ...
        if loss_func == 'gan' or loss_func == 'dragan':
            fake_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(fake[i]),
                                                                               logits=fake[i]))
...
def discriminator_loss(loss_func: str, real: list, fake: list) -> int:
    loss = []
    real_loss = 0
    fake_loss = 0
    ...
    for i in range(2):
        ...
        if loss_func == 'gan' or loss_func == 'dragan':
            real_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(real[i]),
                                                                               logits=real[i]))
            fake_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(fake[i]),
                                                                               logits=fake[i]))
...
def regularization_loss(scope):
    regularization_collection = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    
    loss = []
    for item in regularization_collection:
        if scope in item.name:
            loss.append(item)
